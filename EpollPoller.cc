...
bool isConnectionClose(int sockfd)
{
	...
	cout<<"nread=recvPeek nread:" <<nread<<endl;
	if(-1 == nread)
	{
		perror("recvPeek");
		exit(EXIT_FAILURE);
	...
	else
	{
		...
		for(int idx = 0; idx != nready; ++idx)
		{
			...
			else if(_eventList[idx].data.fd == _eventfd)
			{
				if(_eventList[idx].events & EPOLLIN)
				{
					printf("Epoll wait receive signal \n");
					handleRead();
					doPendingFunctors();
				}
			}
...
void EpollPoller::handleConnection()
{
	...
	pConn->setCloseCallback(_onClose); _connMap[peerfd] = pConn; //给客户端发一些信息 onConnection();
